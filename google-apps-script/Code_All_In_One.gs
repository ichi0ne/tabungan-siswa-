 ...
 * =========================================================================================
 * Seluruh kode backend digabung menjadi 1 file ini agar Anda cukup menyalin (copy-paste)
 * 1 file ini saja ke dalam Google Apps Script editor tanpa perlu membuat banyak file terpisah.
 *
 * CARA PEMASANGAN:
 * 1. Buka Google Sheets baru di https://sheets.new
 * 2. Klik menu 'Extensions' (Ekstensi) > 'Apps Script'.
 * 3. Hapus semua kode default, lalu tempelkan (PASTE) SELURUH isi file ini ke editor.
 * 4. Klik ikon Save (Disket) / Ctrl + S.
 * 5. Klik menu Deploy > New deployment > Select type: Web app.
 *    - Description: API Tabungan Siswa
 *    - Execute as: Me (email akun Anda)
 *    - Who has access: Anyone (Siapa saja)
 * 6. Klik Deploy > Authorize access (Izinkan akses Google).
 * 7. Salin URL Web App yang berakhiran /exec dan masukkan ke menu Pengaturan di Aplikasi Tabungan.
 * =========================================================================================
 */
...
// =========================================================================================
// 2. FUNGSI UTILITY & DATABASE HELPERS
// =========================================================================================
...
// =========================================================================================
// 3. INISIALISASI DATABASE & STRUKTUR SHEET OTOMATIS
// =========================================================================================
function setupDatabase() {
  ...
  if (sheetTransaksi.getLastRow() === 1) {
    ...
    sheetTransaksi.appendRow(["TRX-001", "ST-20260827-0001", todayStr, timeStr, "SISWA-001", "2025001", "Muhammad Rizky Pratama", "KLS-01", "SETORAN", 350000, 0, 350000, "Setoran awal tabungan tahun ajaran baru", "USR-ADMIN-01", "Administrator Utama", "AKTIF", now]);
    sheetTransaksi.appendRow(["TRX-002", "WD-20260827-0002", todayStr, timeStr, "SISWA-001", "2025001", "Muhammad Rizky Pratama", "KLS-01", "PENARIKAN", 50000, 350000, 300000, "Pembelian buku paket sekolah", "USR-BENDAHARA-01", "Ibu Siti Rahmawati, S.Pd", "AKTIF", now]);
    sheetTransaksi.appendRow(["TRX-003", "ST-20260827-0003", todayStr, timeStr, "SISWA-002", "2025002", "Siti Aisyah Azzahra", "KLS-01", "SETORAN", 500000, 0, 500000, "Setoran tabungan rutin", "USR-ADMIN-01", "Administrator Utama", "AKTIF", now]);
  }
...
function handleLogin(payload) {
  ...
  // Cek NIS Siswa jika login siswa
  ...
  return { success: false, message: "Username atau NIS tidak ditemukan dalam sistem." };
}
...
function createUser(payload, operatorUser) {
  ...
  logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Admin", "CREATE_USER", "PENGGUNA", idUser, "Membuat pengguna: " + username);

  return { success: true, message: "Pengguna baru berhasil ditambahkan.", data: { id_user: idUser, username: username } };
}
...
function updateUser(payload, operatorUser) {
  ...
  logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Admin", "UPDATE_USER", "PENGGUNA", idUser, "Memperbarui pengguna " + username);
...
function deleteUser(payload, operatorUser) {
  ...
  if (payload.permanent === true) {
    ...
    logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Admin", "DELETE_USER", "PENGGUNA", idUser, "Menghapus permanen akun pengguna: " + username);
  ...
  logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Admin", "UPDATE_USER", "PENGGUNA", idUser, "Menonaktifkan akun pengguna: " + username);

  return { success: true, message: "Akun pengguna '" + username + "' berhasil dinonaktifkan." };
}

// =========================================================================================
// 5. MASTER DATA SISWA & SALDO
// =========================================================================================
...
function createSiswa(payload, operatorUser) {
  ...
  logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Petugas", "CREATE_SISWA", "SISWA", idSiswa, "Mendaftarkan siswa: " + namaSiswa);
...
function updateSiswa(payload, operatorUser) {
  ...
  logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Petugas", "UPDATE_SISWA", "SISWA", idSiswa, "Memperbarui siswa: " + namaSiswa);
...
function getKelas(params) {
  ...
  if (lastRowSiswa > 1) {
    ...
    for (var s = 0; s < siswaValues.length; s++) {
      ...
      if (st === "AKTIF") countSiswaMap[kId] = (countSiswaMap[kId] || 0) + 1;
    }
  }
...
function createKelas(payload, operatorUser) {
  ...
  logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Petugas", "CREATE_KELAS", "KELAS", idKelas, "Menambahkan kelas: " + namaKelas);

  return { success: true, message: "Kelas berhasil ditambahkan.", data: { id_kelas: idKelas, nama_kelas: namaKelas } };
}
...
function updateKelas(payload, operatorUser) {
  ...
  logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Petugas", "UPDATE_KELAS", "KELAS", idKelas, "Memperbarui kelas: " + namaKelas);
...
function deleteKelas(payload, operatorUser) {
  ...
  if (payload.permanent === true) {
    sheet.deleteRow(rowIndex);
    logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Admin", "DELETE_KELAS", "KELAS", idKelas, "Menghapus permanen kelas: " + namaKelas);
    return { success: true, message: "Kelas '" + namaKelas + "' berhasil dihapus permanen." };
  }

  sheet.getRange(rowIndex, 7).setValue("NONAKTIF");
  logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", operatorUser ? operatorUser.nama : "Petugas", "UPDATE_KELAS", "KELAS", idKelas, "Menonaktifkan kelas: " + namaKelas);

  return { success: true, message: "Kelas '" + namaKelas + "' berhasil dinonaktifkan." };
}

// =========================================================================================
// 7. TRANSAKSI (SETORAN, PENARIKAN, PEMBATALAN / REVERSAL, BUKU TABUNGAN)
// =========================================================================================
function createSetoran(payload, operatorUser) {
  ...
  if (nominal <= 0) return { success: false, message: "Nominal setoran harus lebih besar dari 0 (Nol)." };
...
function cancelTransaksi(payload, operatorUser) {
  ...
  try {
    ...
    var saldoRow = findRowById(sheetSaldo, 1, idSiswa);
    if (saldoRow === -1) return { success: false, message: "Data saldo siswa tidak ditemukan." };
    ...
    if (jenisTrx === "SETORAN") {
      if (currentSaldo < nominal) {
        return {
          success: false,
          message: "Tidak dapat membatalkan setoran karena saldo siswa saat ini (" + formatCurrency(currentSaldo) + ") lebih kecil dari nominal setoran."
        };
      }
    ...
    logActivity(operatorUser ? operatorUser.id_user : "SYSTEM", cancellerName, "CANCEL_TRANSAKSI", "TABUNGAN", noTrx, "Membatalkan " + jenisTrx + " senilai " + formatCurrency(nominal) + " untuk " + namaSiswa);

    return {
      success: true,
      message: "Transaksi " + noTrx + " berhasil dibatalkan.",
      data: { no_transaksi: noTrx, saldo_baru: newSaldo }
    };
  } catch (err) {
...
// =========================================================================================
// 8. DASHBOARD, LAPORAN & AUDIT TRAIL
// =========================================================================================
...
// =========================================================================================
// 9. ROUTER HTTP REQUEST (doGet & doPost)
// =========================================================================================
function doGet(e) {
  try {
